...
ecd_total() {
    ...
    errfile="$dir_path/$setting/coreutils.ecd.$max_state.err.log"
    outfile="$dir_path/$setting/coreutils.ecd.$max_state.out.log"
    eval "rm $errfile"
    eval "rm $outfile"
    for binname in $all
    do
...
zip_total() {
    ...
    zip_file="$dir_path/$setting/coreutils.$ecddir.zip"
    cur_dir=`pwd`
